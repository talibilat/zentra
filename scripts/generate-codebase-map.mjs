...
const featureDefinitions = [
  ...
  ["attention", "Attention & decisions", "Globally unique attention, single-reservation approvals, advisory warnings, and atomic scope admission", "Immutable question or approval packets, globally reserved attention identity, exact run approval request, affected scopes, run revision, actor identity, and audit evidence.", "One replayable accepted, rejected, expired, stale, or advisory resolution with permanent identity/request reservations, independently retained attempts, one durable scope-admission ordering point, and exact run readiness.", "Every source atomically reserves its encoded attention identity; advisory sources remain outside the material index; approval acceptance CAS binds accepted event identity, reservation consumption, index resolution, and run readiness."],
  ...
  ["scheduling", "Daemon scheduling & leases", "Durable fair dispatch and repository-wide capacity", "Admission evidence, independently issued exact grant, bounded resources and budget, workspace, capabilities, platform, and process incarnation.", "Ready or blocked projection, exact dispatch intent, task and worker leases, backpressure, reconciliation, or canonical worker outcome.", "Repository-keyed control and grant streams atomically linearize single-use authority and global reservations; fenced daemons, jittered 60-second heartbeats with safe lease margin, usage receipts, and minimal-environment process supervision prevent oversubscription and unsafe redispatch."],
  ...
  ["observability", "AgentTrail projection", "Redacted rebuildable operational projection", "Stored journal events and durable projection claims.", "Agent Tail 1.x envelopes in checksummed immutable segments, hashed withholding alerts, exact exports, and reconnectable live tail.", "Projection failure never rolls back authoritative journal state; credential-shaped fields fail closed, cursor recovery is idempotent, and repair rebuilds from journal truth into a new private directory."],
  ["soak", "Soak conformance", "Production 20-40 worker stability and recovery evidence", "A frozen profile, seed, externally trusted signing-key digest, bounded capacities, fault matrix, canonical output directory, and optional resume request.", "An atomic Ed25519-signed machine report plus recomputed journal, AgentTrail, archive, and evidence-file digests.", "Two accelerated issue-101 production waves and one unique-worker scheduler fleet exercise real pods, claims, supervised writers, Git integration, AgentTrail, SIGKILL recovery, cursor rebuild, retention maintenance, bounded samples, and resumable operation intents across 1,440 ticks."],
...
const featureByDirectory = new Map([
  ["cli", "entrypoints"], ["contracts", "contracts"], ["journal", "journal"], ["attention", "attention"], ["tasks", "tasks"],
  ["scheduling", "scheduling"], ["leases", "scheduling"],
  ...
  ["observability", "observability"], ["runtime", "runtime"], ["soak", "soak"], ["fixtures", "fixtures"],
]);
