...
export class ChromiumWorkflowDriver {
  ...
  async inspectHostileSource(runId: string): Promise<ChromiumAcceptanceResult> {
    ...
    await evaluate(this.cdp, `window.scrollTo(0,0)`);
    const viewport = await evaluate<ChromiumAcceptanceResult["viewport"]>(this.cdp, `(()=>{const width=innerWidth;const documentWidth=document.documentElement.scrollWidth;const offenders=[];if(documentWidth>width+1){for(const element of document.body.querySelectorAll("*")){if(!(element instanceof HTMLElement)||element.offsetParent===null||element.classList.contains("skip"))continue;const rect=element.getBoundingClientRect();if(rect.right>width+1||rect.left<-1)offenders.push((element.id?"#"+element.id:element.tagName.toLowerCase()+"."+[...element.classList].join("."))+":"+Math.round(rect.left)+".."+Math.round(rect.right))}}return {width,documentWidth,offenders:offenders.slice(0,20)}})()`);
