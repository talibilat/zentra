...
`, { mode: 0o755 });
    ...
    const installedMilestone = await run(binary, [
      ...
      "--harness", "opencode",
      "--harness-executable", realpathSync.native(milestoneOpenCode),
      "--harness-home", realpathSync.native(milestoneOpenCodeHome),
      "--harness-sha256", createHash("sha256").update(readFileSync(milestoneOpenCode)).digest("hex"),
      "--harness-version", "package-opencode 1",
    ...
    const rejectedLegacy = await runNonzero(binary, [
      ...
      "--provider", legacyProvider, "--harness", "opencode", "--harness-executable", realpathSync.native(milestoneOpenCode),
      "--harness-home", realpathSync.native(milestoneOpenCodeHome), "--agent-tail-jsonl", legacyTrace,
      "--harness-sha256", createHash("sha256").update(readFileSync(milestoneOpenCode)).digest("hex"),
      "--harness-version", "package-opencode 1",
      "--file", "greeting.txt",
