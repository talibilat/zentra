...
describe("canonical AgentTrail fleet projection", () => {
  ...
  it("uses exact acquired intervals, unique workers, and clears backpressure on dispatch and release", () => {
    ...
    const events = [
      ...
      event(6, "scheduler.resources_acquired", { taskId: "task-a", resources: workerTask.resources, acquiredAtMs: 6 }),
      event(7, "scheduler.budget_acquired", { taskId: "task-a", budget: workerTask.budget, acquiredAtMs: 7 }),
